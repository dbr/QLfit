...
int main(int numArgs, char* args[])
{
   ...
   if (fitToCsv)
   {
      std::fstream file(in.c_str(), std::ios::in | std::ios::binary);
      Decode decode;
      ...
      broadcaster.AddListener((MesgDefinitionListener&) mesgWriter);
      broadcaster.AddListener((MesgListener&) mesgWriter);
   ...
   {
      Encode encode;
      std::fstream inFile(in.c_str(), std::ios::in);
      std::fstream outFile(out.c_str(), std::ios::out | std::ios::in | std::ios::binary | std::ios::trunc);
      
      encode.Open(outFile);

      if (!CSVReader::Read(inFile, encode, encode))
